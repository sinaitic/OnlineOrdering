...
window.G_V = {"pTitle": "温馨提示", "bimg": "http://qzonestyle.gtimg.cn/campus_v2/ac/b.gif"};
...
window.XY_STYLE = (function () {
    function appendPad() {
        ...
        var _h = '<div class="const_set" id="stylePad"><p class="title">皮肤</p><div class="skins clearfix">';
...
window.html_verifycode_frame = function (title, content, _callback) {
    ...
    var html = '<div class="verifycode">'
        ...
        + '<input id="verifycode_div" name="verifycode_div" type="text" size=4 class="verifycode" maxlength="4" style="ime-mode:disabled;" />不区分大小写</label>'
        + '</td>'
        + '</tr>'
        + '<tr>'
        + '<td></td>'
        ...
        + '<a href="javascript:_nchangeImg(\'loginVerifyImg\',\'verifycode\');" tabindex="-1">看不清,换一张</a>'
        + '</td>'
        + '</tr>'
...
window.global_frame_new = function (title, html, option) {
    ...
    if (w == undefined) {
        ...
        w.innerHTML = '<div id="' + wid + '_wrap" class="layer_global_main ' + op.wrap_class + '">' + (op.show_title ? '<div id="' + wid + '_head" class="layer_global_title">' + hidden_focus + '<h3>' + title + '</h3><button title="关闭" href="javascript:void(0)" id="' + wid + '_close"><span class="none">&#9587;</span></button></div>' : '')
...
window.add_friends_frame = function (u, ref, option) {
    ...
    function cb(d) {
        ...
        if (d.err) {
            return html_error_frame('温馨提示', d.msg);
        }
        if (d.confirm) {
            eval(d.cb);
...
window.pop_add_friends_frame = function (u_hash, u_info, ref, option) {
    ...
    return html_frame_new('将 ' + u_info.name + ' 加为好友 ', '<iframe id="id_add_friend" name="id_add_friend" frameborder="0" scrolling="no" width="100%" height="158px" src="http://' + DOMAINS.MAIN + '/html/frame/addfriends.html"></iframe>', add_friends, null, {
        wid: "af_box",
        'submit_class': 'bt_tx2',
        div_width: 450
    });
...
window.do_del_friends = function (u, _callback) {
    function cb(txt) {
        var tmp = txt.split('|');
        if (tmp.length != 2) {
            html_error_frame('温馨提示', '解除关系失败，系统错误');
            return false;
        }
        if (tmp[0] != '0') {
            html_error_frame('温馨提示', tmp[1]);
            return false;
        }
        if (_callback) {
            _callback(u);
...
window.is_sendscrip = function (u, ref) {
    var cb = function (d) {
        if (d.err == 0) {
            ...
            return html_confirm_frame('温馨提示', d.msg, function () {
...
window.doSendPoke = function (u, id, type) {
    ...
    var cb = function () {
        ...
        if (!data || data.xml == "") {
            html_error_frame('温馨提示', "发送失败");
            return;
        }
        if (data.getElementsByTagName("error").length > 0) {
            var er = data.getElementsByTagName("error")[0];
            if (er.getAttribute("type") == "login") html_error_frame('温馨提示', "您已登录超时，请重新登录"); else {
                if (er.textContent) html_error_frame('温馨提示', er.textContent); else html_error_frame('温馨提示', er.text);
            }
    ...
    loadXMLAsyncNoCache("sendPoke", "http://" + pokeDomain + "/cgi-bin/sendpoke", cb, function () {
        html_error_frame('温馨提示', "很抱歉,目前网络繁忙,请稍后再试");
    }, url);
};
